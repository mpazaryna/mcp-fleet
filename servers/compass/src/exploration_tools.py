...
class StartExplorationInputSchema(BaseModel):
    ...
    focus_area: str | None = Field(default=None, description="Specific area to explore")
    projects_dir: str | None = Field(
        default=DEFAULT_WORKSPACE, description="Projects directory"
    )
...
class SaveExplorationSessionInputSchema(BaseModel):
    ...
    project_name: str = Field(description="Project name")
    conversation_content: str = Field(
        description="The exploration conversation content"
    )
    session_summary: str | None = Field(
        default=None, description="Optional summary of the session"
    )
    projects_dir: str | None = Field(
        default=DEFAULT_WORKSPACE, description="Projects directory"
    )
...
# Tool handlers
async def start_exploration_handler(args: dict[str, Any]) -> dict[str, Any]:
    """Start an exploration session"""
    validated_args = StartExplorationInputSchema(**args)

    try:
        projects_dir = Path(validated_args.projects_dir or DEFAULT_WORKSPACE)
        ...
        try:
            ...
            exploration_tasks = [
                Task(completed=False, text=task) for task in DEFAULT_EXPLORATION_TASKS
            ]
        ...
        if validated_args.focus_area:
            ...
            incomplete_tasks = [
                task for task in exploration_tasks if not task.completed
            ]
            if incomplete_tasks:
...
**Remember:** The goal of exploration is deep understanding, not quick answers. Take time to really understand the nuances and complexities."""
        ...
        logger.info(
            f"Started exploration session {session_number} for project: {validated_args.project_name}"
        )
...
async def save_exploration_session_handler(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Saved exploration session {metadata.session_count} for project: {validated_args.project_name}"
        )
...
async def get_project_context_handler(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            tasks_data = await project_manager.parse_tasks_file()
            questions_remaining = [
                task.text for task in tasks_data.exploration_tasks if not task.completed
            ]
        except Exception:
            pass
...
async def complete_exploration_phase_handler(args: dict[str, Any]) -> dict[str, Any]:
...
"""
        ...
        logger.info(
            f"Completed exploration phase for project: {validated_args.project_name}"
        )
