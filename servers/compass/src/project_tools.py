...
async def list_projects_handler(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if not projects_dir.exists():
            return ListProjectsOutputSchema(projects=[], total=0).model_dump()
...
async def get_project_status_handler(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if metadata.current_phase == "exploration":
            if total_tasks == 0:
                ...
                next_action = (
                    "Continue exploration sessions to complete remaining tasks"
                )
                recommendations = [
