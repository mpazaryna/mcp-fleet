...
class TestMemryE2E:
    ...
    async def test_memory_creation_workflow(self):
        ...
        async with client.connect() as mcp:
            # Create a memory
            create_result = await mcp.call_tool("create_memory", {
            ...
            assert create_result.get("structuredContent", {}).get("success") == True
            ...
            assert content["content"] == "This is a test memory created during E2E testing."
            assert "test" in content["tags"]
            assert "e2e" in content["tags"]
            print(f"✅ Verified JSON file content")
    ...
    async def test_memory_search_workflow(self):
        ...
        async with client.connect() as mcp:
            ...
            # Test search by tags
            search_result = await mcp.call_tool("search_memories", {
                "tags": ["ai"]
            })
            
            assert search_result.get("structuredContent", {}).get("success") == True
            found_memories = search_result.get("structuredContent", {}).get("memories", [])
            assert len(found_memories) == 1
            assert found_memories[0]["title"] == "AI Research Notes"
            print(f"✅ Tag search found correct memory")
            
            # Test content search
            content_search_result = await mcp.call_tool("search_memories", {
                "content_search": "protocol"
            })
            
            assert content_search_result.get("structuredContent", {}).get("success") == True
            protocol_memories = content_search_result.get("structuredContent", {}).get("memories", [])
            assert len(protocol_memories) == 1
            assert protocol_memories[0]["title"] == "Protocol Design"
            print(f"✅ Content search found correct memory")
    ...
    async def test_memory_stats_workflow(self):
        ...
        async with client.connect() as mcp:
            ...
            test_memories = [
                {"title": "Memory 1", "content": "Content 1", "source": "claude-desktop", "tags": ["work", "ai"]},
                {"title": "Memory 2", "content": "Content 2", "source": "claude-code", "tags": ["work", "development"]},
                {"title": "Memory 3", "content": "Content 3", "source": "claude-desktop", "tags": ["personal", "ai"]}
            ]
            ...
            assert stats_result.get("structuredContent", {}).get("success") == True
    ...
    async def test_list_all_memories_workflow(self):
        ...
        async with client.connect() as mcp:
            ...
            assert list_result.get("structuredContent", {}).get("success") == True
    ...
    async def test_error_handling(self):
        ...
        async with client.connect() as mcp:
            ...
            try:
                ...
                assert result.get("structuredContent", {}).get("success") == False
    ...
    async def test_persistence_across_restarts(self):
        ...
        async with client2.connect() as mcp:
            ...
            persistent_memory = next(
                (m for m in memories if m["title"] == "Persistent Memory"), 
                None
            )
