...
from compass.src.managers.project_manager import (
    ProjectManager, Task, TasksData, ProjectMetadata
)
...
"""
    ...
    assert tasks_data.exploration_tasks[0].completed == True
    assert tasks_data.exploration_tasks[0].text == "Understand the core problem"
    assert tasks_data.exploration_tasks[1].completed == False
...
async def test_check_exploration_completion(temp_project):
    ...
    assert completion_status["is_complete"] == True  # 80% >= 80% threshold
