...
class TestMemoryTools:
    ...
    async def test_list_all_memories_handler(self):
        ...
        memory1 = CreateMemoryInputSchema(
            title="Memory 1", content="Content 1", source="claude-code", tags=["test"]
        )
    ...
    async def test_content_preview_in_search(self):
        ...
        long_content = (
            "This is a very long piece of content that should be truncated in the preview. "
            * 10
        )
    ...
    async def test_search_with_date_range(self):
        ...
        search_input = SearchMemoriesInputSchema(date_from=today, date_to=today)
        result = await search_memories_handler(search_input)
