...
async def test_create_project_tide(temp_storage):
    ...
    input_data = CreateTideInput(name="Website Redesign", flow_type="project")
...
async def test_list_tides(temp_storage):
    ...
    await temp_storage.create_tide(
        CreateTideInput(name="Daily Flow", flow_type="daily")
    )
    await temp_storage.create_tide(
        CreateTideInput(name="Weekly Review", flow_type="weekly")
    )
    await temp_storage.create_tide(
        CreateTideInput(name="Project Work", flow_type="project")
    )
