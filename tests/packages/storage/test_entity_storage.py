...
class SampleEntity(BaseModel):
    ...
    def create_with_timestamp(
        cls, id: str, name: str, value: int, metadata: dict[str, Any] | None = None
    ):
...
class TestEntityStorage:
    ...
    async def storage(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            backend = JSONFileBackend(
                storage_path=Path(tmpdir), entity_type=SampleEntity
            )
            storage = create_entity_storage(entity_type=SampleEntity, backend=backend)
            yield storage
    ...
    async def test_create_entity(self, storage):
        ...
        entity = SampleEntity.create_with_timestamp(
            id="test-1", name="Test Entity", value=42, metadata={"key": "value"}
        )
    ...
    async def test_get_entity(self, storage):
        ...
        entity = SampleEntity.create_with_timestamp(
            id="test-2", name="Get Test", value=100
        )
    ...
    async def test_list_entities(self, storage):
        ...
        for i in range(3):
            entity = SampleEntity.create_with_timestamp(
                id=f"list-test-{i}", name=f"Entity {i}", value=i * 10
            )
            await storage.create(entity)
    ...
    async def test_update_entity(self, storage):
        ...
        entity = SampleEntity.create_with_timestamp(
            id="update-test", name="Original Name", value=50
        )

        await storage.create(entity)

        # Update the entity
        updates = {"name": "Updated Name", "value": 75}
    ...
    async def test_delete_entity(self, storage):
        ...
        entity = SampleEntity.create_with_timestamp(
            id="delete-test", name="To Delete", value=999
        )
...
def test_create_entity_storage():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        backend = JSONFileBackend(storage_path=Path(tmpdir), entity_type=SampleEntity)
