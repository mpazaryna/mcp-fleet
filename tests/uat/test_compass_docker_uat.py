...
import logging
from mcp_docker_client import MCPDockerClient, COMPASS_SERVER, create_test_workspace
...
def test_compass_project_workflow():
    ...
    with MCPDockerClient(compass_config, debug=True) as client:
        ...
        expected_tools = [
            "init_project", "start_exploration", "save_exploration_session",
            "complete_exploration_phase", "generate_specification", 
            "start_execution", "list_tasks", "update_task_status"
        ]
        ...
        init_result = client.call_tool("init_project", {
        ...
        with open(config_file, 'r') as f:
            config = json.load(f)
        assert config["phase"] == "exploration", f"Expected exploration phase, got {config['phase']}"
        
        print(f"✅ Project initialized at {project_path}")
        
        # Test 2: Start exploration session
        print("\n🎬 Test 2: Start exploration session...")
        exploration_result = client.call_tool("start_exploration", {
            "project_name": "uat-demo-app",
        ...
        print("\n🎬 Test 3: Save exploration session...")
        session_result = client.call_tool("save_exploration_session", {
            "project_name": "uat-demo-app",
        ...
        print("\n🎬 Test 4: Complete exploration phase...")
        complete_result = client.call_tool("complete_exploration_phase", {
            "project_name": "uat-demo-app",
        ...
        spec_result = client.call_tool("generate_specification", {
            "project_name": "uat-demo-app",
        ...
        with open(spec_file, 'r') as f:
            spec_content = f.read()
        assert "authentication" in spec_content.lower(), "Specification missing key features"
        assert "offline" in spec_content.lower(), "Specification missing offline requirements"
        
        print(f"✅ Specification generated: {spec_file}")
        
        # Test 6: Start execution phase
        print("\n🎬 Test 6: Start execution phase...")
        execution_result = client.call_tool("start_execution", {
            "project_name": "uat-demo-app"
        })
        ...
        with open(config_file, 'r') as f:
            config = json.load(f)
        assert config["phase"] == "execution", f"Expected execution phase, got {config['phase']}"
        
        # Test 7: List and manage tasks
        print("\n🎬 Test 7: List execution tasks...")
        tasks_result = client.call_tool("list_tasks", {
            "project_name": "uat-demo-app"
        })
        ...
        if "content" in tasks_result and tasks_result["content"]:
            ...
            if "Task ID:" in task_content:
                ...
                if task_id:
                    update_result = client.call_tool("update_task_status", {
                        "project_name": "uat-demo-app",
                        "task_id": task_id,
                        "status": "in_progress",
        ...
        # Count files created
        total_files = len(list(project_path.rglob("*.md"))) + len(list(project_path.rglob("*.json")))
...
def test_compass_error_handling():
    ...
    with MCPDockerClient(compass_config) as client:
        ...
        try:
            client.call_tool("start_execution", {
                "project_name": "phase-test"
            })
        ...
        try:
            client.call_tool("start_exploration", {
                "project_name": "non-existent-project",
                "exploration_focus": "test"
            })
