...
class UATTestRunner:
    ...
    def check_prerequisites(self) -> bool:
        ...
        missing_images = [server for server, available in image_status.items() if not available]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_server):
                ...
                try:
                    ...
                    result = UATTestResult(server, self.test_configs[server]["test_file"])
                    result.start_time = datetime.now()
                    result.end_time = datetime.now()
    ...
    def run(self, servers: Optional[List[str]] = None, parallel: bool = False) -> bool:
        """Run UAT tests"""
        ...
        if servers is None:
            ...
            image_status = self.check_docker_images()
            servers = [server for server, available in image_status.items() if available]
