...
def test_memry_core_functionality():
    ...
    with MCPDockerClient(memry_config, debug=True) as client:
        ...
        print("\n🎬 Test 2: List all memories...")
        list_result = client.call_tool("list_memories", {
            "limit": 10,
        ...
        if "content" in list_result:
            ...
            for memory in memories_data:
                assert memory["title"] in list_content, f"Memory not found in list: {memory['title']}"
        ...
        # Search by content
        search_result = client.call_tool("search_memories", {
        ...
        # Search by tags
        tag_search = client.call_tool("search_memories", {
            "query": "technical",
            "search_in": ["tags"],
            "limit": 5
        })
        ...
        # Search by source
        source_search = client.call_tool("search_memories", {
            "query": "research",
            "search_in": ["source"],
            "limit": 5
        })
        ...
        if created_memories and "content" in created_memories[0]:
            ...
            if "memory ID:" in content.lower():
                ...
                if memory_id:
                    get_result = client.call_tool("get_memory", {
                        "memory_id": memory_id
                    })
        ...
        update_test = client.call_tool("create_memory", {
        ...
        export_result = client.call_tool("export_memories", {
            "format": "json",
            "filter": {
        ...
        assert len(memory_files) >= len(memories_data), f"Expected at least {len(memories_data)} files, found {len(memory_files)}"
        ...
        for memory_file in memory_files[:3]:  # Check first 3 files
            ...
            for field in required_fields:
                assert field in memory_data, f"Missing field {field} in {memory_file.name}"
        ...
        print(f"   🔍 Search operations: 3")
        print(f"   📝 Update operations: 1")
        print(f"   📤 Export operations: 1")
...
def test_memry_advanced_features():
    ...
    with MCPDockerClient(memry_config) as client:
        ...
        multi_search = client.call_tool("search_memories", {
        ...
        large_memory = client.call_tool("create_memory", {
        ...
        special_memory = client.call_tool("create_memory", {
        ...
        limited_list = client.call_tool("list_memories", {
            "limit": 2,
            "include_content": False
        })
        
        # List with larger limit
        full_list = client.call_tool("list_memories", {
            "limit": 100,
            "include_content": True
        })
...
def test_memry_error_handling():
    ...
    with MCPDockerClient(memry_config) as client:
        ...
        try:
            client.call_tool("create_memory", {
                "title": "",  # Empty title
